...
class TestNomeDaIesFormater(unittest.TestCase):
    ...
    def test_single_row_with_string(self):
        ...
        self.assertEqual(cleaned_data['Nome da IES'].tolist(), ['University Of Abc'])
    ...
    def test_multiple_rows_with_strings(self):
        ...
        cleaned_data = nome_da_ies_formater(data)
        self.assertTrue('Nome da IES' in cleaned_data.columns)
        self.assertEqual(cleaned_data['Nome da IES'].tolist(), ['University Of Abc', 'Escola De Matemática Aplicada'])
...
class TestAreaDeAvaliacaoLong(unittest.TestCase):
    ...
    def test_mixed_names(self):
        data = pd.DataFrame({'Área de Avaliação': ['Tecnologia Em Redes De Computadores', 'Other', 'Tecnologia Em Análise E Desenvolvimento De Sistemas', 'Another']})
        expected_data = pd.DataFrame({'Área de Avaliação': ['Redes De Computadores', 'Other', 'Desenvolvimento De Sistemas', 'Another']})
        modified_data = area_de_avaliacao_long(data)
        self.assertEqual(modified_data['Área de Avaliação'].tolist(), expected_data['Área de Avaliação'].tolist())
...
class TestAddStateNameToData(unittest.TestCase):
    ...
    def test_all_state_codes(self):
        ...
        gdf = gpd.GeoDataFrame({'codarea': [12, 27, 16, 13, 29, 23, 53, 32, 52, 21, 51, 50, 31, 15, 25, 41, 26, 22, 24, 43, 33, 11, 14, 42, 35, 28, 17], 'geometry': [None] * 27})
        ...
        self.assertEqual(gdf_with_state['Sigla da UF '].tolist(), ['AC', 'AL', 'AP', 'AM', 'BA', 'CE', 'DF', 'ES', 'GO', 'MA', 'MT', 'MS', 'MG', 'PA', 'PB', 'PR', 'PE', 'PI', 'RN', 'RS', 'RJ', 'RO', 'RR', 'SC', 'SP', 'SE', 'TO'])
    ...
    def test_valid_state_codes(self):
        ...
        gdf = gpd.GeoDataFrame({'codarea': [30, 30, 30], 'geometry': [None, None, None]})
